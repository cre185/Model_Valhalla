...
方钧同（前端）：项目框架引入；应用原型设计（部分）；登录和注册页面的功能及实现，个人安全手机绑定修改和安全邮箱绑定修改实现；排行榜主页和功能实现，排行榜模型对抗评测记录实现，排行榜模型详情页面实现及详情项整合；数据集主页和功能实现，数据集详情页面实现及详情项整合；主管评测页面及功能实现；个人信息页面及功能实现，关注模型和数据集列表实现；导航栏站内信完善，夜间模式切换实现
...
钟健坤（后端）：完成了后端的接口提供等必要工作；顺带负责对于接口的单元测试与性能测试；完成了后端部署操作，在部署过程中遇到问题时和前端同学合作解决；进行大部分开发过程文档的编写，如API文档、部署文档、测试文档等。  
...
+ Problem：ts中涉及到较为复杂的异步函数，具体原理的理解和使用方法：

  Solution：

  ##### Promise：

  `Promise` 是 JavaScript 中处理异步操作的一种机制。它表示一个可能会在未来完成的操作。一个 Promise 可以处于三种状态之一：Pending（进行中）、Fulfilled（已完成）或 Rejected（已拒绝）。

  ##### async 函数：

  `async` 关键字用于定义一个异步函数。异步函数返回一个 Promise 对象，其中包含函数执行的结果。`async` 函数内部可以使用 `await` 关键字等待 Promise 的解决。

  ##### await 关键字：

  `await` 关键字只能在 `async` 函数内部使用。它用于等待一个 Promise 对象解决，并返回 Promise 的结果值。

  ##### 异步函数的执行顺序和原理：

  1. 当调用一个 `async` 函数时，它会立即返回一个 Promise 对象，即使函数内部没有异步操作。
  2. 在 `async` 函数内部，使用 `await` 关键字可以等待一个 Promise 的解决，同时不阻塞其他代码的执行。
  3. `async` 函数内部的代码按照顺序执行，但在遇到 `await` 时会将控制权交回给调用者，使得其他代码有机会执行。
  4. 当 `await` 后面的 Promise 解决时，`async` 函数会继续执行后面的代码。
...
### 核心算法  
一个我们使用到的比较有趣的算法是在模型对抗评测中涉及到的Elo评分算法，这一算法用于计算模型之间的实力差距，从而决定对抗评测中的对手匹配。 
...
![34](F:\Model_Valhalla\docs\source\images\34.png)
