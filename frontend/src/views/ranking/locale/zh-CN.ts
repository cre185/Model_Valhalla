export default {
  'ranking.welcome': '欢迎来到排行榜!',
  'ranking.slogan1': '在这里找到最强大的模型',
  'ranking.slogan2': '抑或是为你的模型加冕',
  'ranking.llm.upload': '上传',
  'ranking.llm.search.placeholder': '请输入您希望查看的模型名称',
  'rankings.llm.data.ranking': '综合排名',
  'rankings.llm.data.name': '模型名称',
  'rankings.llm.data.mmluScore': 'MMLU 得分',
  'rankings.llm.data.datasetScore': '数据集综合得分',
  'rankings.llm.data.eloScore': 'elo对抗得分',
  'rankings.llm.data.details': '详情',
  'rankings.llm.data.license': '许可',
  'rankings.llm.details.subscribe.btn': '关注',
  'ranking.actions.setColumn': '列设置',
  'ranking.profile.ranking': '综合排名',
  'ranking.profile.info.description': '模型描述',
  'ranking.profile.info.paper': '相关论文',
  'ranking.profile.info.license': '许可',
  'ranking.profile.info.index': '官方网站',
  'ranking.view.btn': '查看',
  'ranking.profile.discussion.reply': '回复',
  'ranking.profile.discussion.submit': '发布',
  'ranking.profile.discussion.reply.placeholder': '发表你的评论',
  'ranking.profile.discussion.replyEmbed.placeholder': '回复 @',
  'ranking.behaviour.dataset.num': '编号',
  'ranking.behaviour.dataset.name': '名称',
  'ranking.behaviour.dataset.contenttype': '内容',
  'ranking.behaviour.dataset.labelsearch': '标签搜索',
  'ranking.behaviour.dataset.contentsize': '内容量',
  'ranking.behaviour.dataset.createdTime': '创建时间',
  'ranking.behaviour.dataset.score': '得分',
  'ranking.behaviour.dataset.num.default': '请输入数据集编号',
  'ranking.behaviour.dataset.name.default': '请输入数据集名称',
  'ranking.behaviour.dataset.size.max.default': '上限',
  'ranking.behaviour.dataset.size.min.default': '下限',
  'ranking.behaviour.dataset.contenttype.default': '全部',
  'ranking.behaviour.dataset.labelsearch.default': '全部',
  'ranking.behaviour.dataset.score_min.default': '0',
  'ranking.behaviour.dataset.score_max.default': '100',
  'ranking.behaviour.button.filter': '筛选',
  'ranking.behaviour.button.reset': '重置',
  'ranking.behaviour.contentType.mix': '混合题',
  'ranking.behaviour.contentType.subjective': '主观题',
  'ranking.behaviour.contentType.objective': '客观题',
  'ranking.behaviour.table.num': '数据集编号',
  'ranking.behaviour.table.name': '数据集名称',
  'ranking.behaviour.table.contentType': '主要内容',
  'ranking.behaviour.table.contentSize': '内容量',
  'ranking.behaviour.table.createdTime': '创建时间',
  'ranking.behaviour.table.score': '得分',
  'ranking.details.details': '详细信息',
  'ranking.details.datasetScore': '数据集表现',
  'ranking.details.competitionRecords': '对抗记录',
  'ranking.details.discussions': '讨论区',
  'ranking.behaviour.dataset.num.errormsg': '请输入大于0的整数',
  'ranking.behaviour.dataset.size.errormsg': '请输入正整数',
  'ranking.behaviour.dataset.score.errormsg': '得分区间为[0,100]间的整数',
  'ranking.adversarial.selector.model': '对抗模型',
  'ranking.adversarial.selector.model.placeholder': '请选择模型',
  'ranking.adversarial.selector.time': '测评时间',
  'ranking.adversarial.selector.time.lowerBound': '最早时间',
  'ranking.adversarial.selector.time.upperBound': '最晚时间',
  'ranking.adversarial.selector.result': '胜负情况',
  'ranking.adversarial.selector.result.win': '获胜',
  'ranking.adversarial.selector.result.lost': '落败',
  'ranking.adversarial.selector.result.draw': '平局',
  'ranking.adversarial.selector.title.user': '测试用户',
  'ranking.adversarial.selector.title.model': '对抗模型',
  'ranking.adversarial.selector.title.time': '测评时间',
  'ranking.adversarial.selector.title.result': '胜负情况',
  'ranking.adversarial.selector.title.details': '对抗详情',
  'ranking.adversarial.selector.model.all': '全部',
  'ranking.adversarial.detail.round': '选择轮次',
  'ranking.adversarial.detail.question': '测试问题',
  'ranking.adversarial.detail.reply.this': '本模型回复',
  'ranking.adversarial.detail.reply.opposite': '对抗模型回复',
  'ranking.actions.density': '大小',
  'ranking.operation.download': '下载',
  'ranking.actions.refresh': '刷新',
};
